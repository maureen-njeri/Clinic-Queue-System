...
export default function ReceptionistDashboard() {
  ...
  const {
    ...
    fetcher,
    { refreshInterval: 5000 } // auto-refresh every 5s, no flicker
  )
  ...
  const filteredAppointments = useMemo(() => {
    if (!appointments) return [] // ✅ prevent crash while loading
    return appointments.filter((a: Appointment) => {
      ...
      const phoneMatch =
        !filters.searchPhone || a.patient?.phone?.includes(filters.searchPhone)
            ...
            <form onSubmit={handleSubmit} className='space-y-4'>
              <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4'>
                ...
                <FormSelect
                  ...
                  onChange={(e: ChangeEvent<HTMLSelectElement>) =>
                    setForm({ ...form, doctorType: e.target.value })
                  }
                  options={doctorTypes}
                  label='Select Doctor'
                  required
            ...
            <div className='border-2 border-black p-8 space-y-4'>
              <div className='grid grid-cols-2 gap-8'>
                <div>
                  ...
                  <p className='text-lg font-semibold'>
                    {selectedSlip.patient?.fullName}
                  </p>
                </div>
                <div>
                  <p className='text-sm text-gray-600'>Queue Number</p>
                  <p className='text-lg font-semibold'>
                    #{selectedSlip.queueNumber}
                  </p>
                </div>
              </div>
              ...
              <div className='grid grid-cols-2 gap-8'>
                <div>
                  ...
                  <p className='text-lg font-semibold'>
                    {selectedSlip.patient?.phone}
                  </p>
                </div>
                <div>
                  <p className='text-sm text-gray-600'>Email</p>
                  <p className='text-lg font-semibold'>
                    {selectedSlip.patient?.email || 'N/A'}
                  </p>
                </div>
              </div>
              ...
              <div className='grid grid-cols-2 gap-8'>
                <div>
                  ...
                  <p className='text-lg font-semibold'>
                    {selectedSlip.patient?.reason}
                  </p>
                </div>
                <div>
                  <p className='text-sm text-gray-600'>Doctor Type</p>
                  <p className='text-lg font-semibold'>
                    {selectedSlip.patient?.doctorType}
                  </p>
                </div>
              </div>
...
const FormSelect = ({
  ...
  <div className='relative'>
    ...
    <select
      ...
      className={`w-full ${
        icon ? 'pl-10' : 'pl-4'
      } pr-4 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:border-purple-500/50 focus:bg-white/20 transition-all appearance-none`}
    >
