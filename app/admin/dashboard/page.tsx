...
export default function AdminDashboard() {
  ...
  // ✅ Only show loading spinner while verifying
  if (!authReady) {
    return (
  ...
  const {
    ...
    status === 'authenticated' ? '/api/admin/logs' : null,
    fetcher,
    {
      revalidateOnFocus: false,
      revalidateOnReconnect: false,
  ...
  const error = usersError || metricsError || logsError
  const loading = usersLoading || metricsLoading || logsLoading
...
const StatCard = ({
  ...
  <div
    className={`bg-gradient-to-br ${color} bg-opacity-10 backdrop-blur border border-white/10 rounded-xl p-6 hover:border-white/20 transition-all h-full`}
  >
