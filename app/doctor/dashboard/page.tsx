...
export default function DoctorDashboard() {
  ...
  // ✅ Update local state whenever new data comes in
  useEffect(() => {
    if (data && !isLoading) {
  ...
  // ✅ When SWR data updates, store it in appointments state
  useEffect(() => {
    if (data && !isLoading) {
      setAppointments(data)
  ...
  // ✅ Simplified status update using SWR mutate
  const updateStatus = async (id: string, newStatus: string) => {
    try {
      ...
      mutate('/api/appointment') // ✅ refresh SWR data instead of refetching manually
    } catch {
      toast.error('Failed to update status')
    }
  }

  // ✅ Example for save handler (same pattern)
              ...
              filtered.map((appointment) => {
                ...
                return (
                  ...
                  >
                    <div
                      onClick={() =>
                        setExpandedId(isExpanded ? null : appointment._id)
                      }
                    ...
                    {isExpanded && (
                      <div className='border-t border-white/5 p-6 space-y-6 bg-white/5'>
                        <div className='grid grid-cols-2 sm:grid-cols-4 gap-4'>
                          ...
                          <VitalCard
                            label='BP'
                            value={appointment.bloodPressure}
                            unit=''
                          />
                          <VitalCard
                            label='Weight'
                            ...
                            <input
                              ...
                              value={
                                edited[appointment._id]?.followUp
                                  ? edited[appointment._id].followUp.split(
                                      'T'
                                    )[0]
                                  : ''
                              }
                        ...
                        {!isDone && (
                          <div>
                            ...
                            <div className='flex gap-2'>
                              {['waiting', 'in-progress', 'done'].map(
                                (status) => (
                                  <button
                                    key={status}
                                    onClick={() =>
                                      updateStatus(appointment._id, status)
                                    }
