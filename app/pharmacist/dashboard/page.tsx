...
export default function PharmacistDashboard() {
  ...
  // Use SWR to handle fetching + auto refresh every 10s
  const { data, error, isLoading, mutate } = useSWR(
    status === 'authenticated' ? '/api/appointment' : null,
    fetcher,
    { refreshInterval: 10000 } // automatically refetch every 10 seconds
  )

  // When data updates, store prescriptions
  useEffect(() => {
    if (data) {
  ...
  const handlePrint = async (prescription: Prescription) => {
    try {
      ...
      doc.setFontSize(10)
      doc.text('Official Dispensing Document', pageWidth / 2, 28, {
        align: 'center',
      })
      ...
      doc.setFillColor(
        prescription.dispensed ? 46 : 230,
        prescription.dispensed ? 185 : 126,
        prescription.dispensed ? 89 : 34
      )
  ...
  const handleDownloadAll = async () => {
    try {
      ...
      doc.setTextColor(255, 255, 255)
      doc.setFontSize(18)
      doc.text('PHARMACY PRESCRIPTIONS REPORT', pageWidth / 2, 16, {
        align: 'center',
      })
      ...
      doc.setFontSize(10)
      doc.text(
        `Generated: ${new Date().toLocaleString()} | Total: ${
      ...
      prescriptions.forEach((prescription, index) => {
        ...
        doc.setFontSize(9)
        doc.text(
          `Phone: ${prescription.patient?.phone} | Doctor: ${prescription.patient?.doctorType}`,
          20,
          y + 7
        )
        doc.text(
        ...
        doc.text(
          `Rx: ${prescription.prescription?.substring(0, 60)}${
            prescription.prescription && prescription.prescription.length > 60
              ? '...'
              : ''
          }`,
          20,
  ...
  const filtered = useMemo(() => {
    ...
    return result.sort((a, b) => {
      if (sortBy === 'name') {
        return (a.patient?.fullName || '').localeCompare(
          b.patient?.fullName || ''
        )
      }
      return (
  ...
  return (
    <div className='min-h-screen w-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 overflow-x-hidden'>
      ...
      <div
        className={`fixed left-0 top-0 w-64 h-screen bg-slate-800/50 backdrop-blur-xl border-r border-white/5 p-6 flex flex-col transition-all z-40 ${
          ...
          <div className='bg-slate-800/30 backdrop-blur-xl border border-white/10 rounded-2xl p-6'>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4'>
              ...
              >
                ...
                <option value='dispensed' className='bg-slate-800'>
                  Dispensed Only
                </option>
              </select>
              ...
              filtered.map((prescription, index) => {
                ...
                return (
                  ...
                  >
                    <div
                      onClick={() =>
                        setExpandedId(isExpanded ? null : prescription._id)
                      }
                      ...
                      <div className='border-t border-white/5 p-6 space-y-6 bg-white/5'>
                        ...
                        <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
                          ...
                          <InfoField
                            ...
                            value={prescription.patient?.doctorType}
                          />
                          <InfoField
                            label='Status'
...
const QuickStat = ({ label, value, icon, color }: any) => {
  ...
  return (
    <div
      ...
      } p-6 rounded-xl text-white shadow-lg hover:shadow-xl transition-all`}
    >
      <div className='flex items-center justify-between'>
        <div>
