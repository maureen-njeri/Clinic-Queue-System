...
export default function NurseDashboard() {
  ...
  const { data, error, isLoading, mutate } = useSWR(
    ...
    fetcher,
    {
      refreshInterval: 5000, // auto-refresh every 5s (background)
  ...
  const filteredAppointments = useMemo(() => {
    return appointments
      .filter((a) => {
        ...
        const matchesPhone =
          !filters.phone || a.patient?.phone?.includes(filters.phone)
        const matchesDoctor =
          !filters.doctor ||
                      ...
                      <div className='border-t border-white/5 p-6 space-y-6 bg-white/5'>
                        ...
                        <div>
                          ...
                          <div className='flex gap-2'>
                            {(['waiting', 'in-progress'] as const).map(
                              (status) => (
                                <button
                                  key={status}
                                  onClick={() =>
